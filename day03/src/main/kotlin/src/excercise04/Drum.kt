...
class Drum<T : Bullet>() {
    ...
    fun shoot(): Boolean {
        ...
        return if (bullet != null && bullet.isShotPossible) {
            ...
            println("Click!" + let {
                when {
                    bullet?.isDamp == true -> " A damp one"
                    bullet?.isShot == true -> " A shot one"
                    else -> ""
                }
            })
            false
    ...
    fun deleteDefectBullets() {
        var current = drum.head

        repeat(DRUM_SIZE) {
            if (current?.value?.isDamp == true || current?.value?.isShot == true) {
                current?.value = null
            }
            current = current?.next
        }
