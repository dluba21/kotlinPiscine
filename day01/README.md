...
**Advice!** In the `src/exercise1` folder, there is some code you can start with. In the `ex01.data.Incident.kt` file, there are two classes, united by meaning. Note, that in Kotlin you can place different entities (classes, functions, etc.) in one file.
...
### ex01.data.Incident description:
- An incident is determined by two integer coordinates on the coordinate plane (in `ex01.data.Incident.kt` they are described as `x` and `y`)
...
- Enter the coordinates of the point of the incident. When the program creates an ex01.data.Incident object, it must randomly take a description and a phone number from corresponding arrays in `utils` folder 
